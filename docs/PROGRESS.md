...
- [x] イベント編集機能 Ver1.00 完成（2025-12-27）
  - 保存ボタンの処理中に非活性化（二重クリック防止）
  - 保存ボタンのテキストを「保存中...」に変更して処理中であることを明示
  - 保存完了後のモーダルで「閉じる」と「一覧に戻る」の2択を提供
  - セリフ一覧の最後にも「セリフを追加」ボタンを配置（UX改善）
  - セリフ追加処理を共通関数化して、ヘッダーと最後のボタンの両方から使用可能に
- [x] イベント再生機能（プレビュー機能）実装（2025-12-27）
  - トップページに「イベント再生」ボタンを追加
  - イベント選択画面を作成（イベント一覧から選ぶ）
  - イベント編集画面に「イベントを再生してみる」ボタンを追加（モーダル表示）
  - EventPreview.jsコンポーネントを作成（参考: pawapoke-MFver）
    - タイプライター風のテキスト表示機能
    - 左右のキャラクター画像表示機能
    - 前へ/次へボタンでセリフを移動
    - 進捗表示（現在のセリフ数/総セリフ数）
  - EventSelect.jsコンポーネントを作成（再生用のイベント選択画面）
  - プレビュー用のCSSスタイルを追加（ゲーム画面風のデザイン）
  - フルスクリーン表示とモーダル表示の両方に対応
- [x] Ver2.00 完成（2025-12-27）
  - イベント再生画面のUI改善
    - レイアウト構造を修正（ヘッダー → ゲーム画面 → セリフエリア → 操作ボタン）
    - セリフエリアをゲーム画面の外（下部）に配置
    - キャラクター画像のサイズを固定（PC: 300px × 300px、タブレット: 250px × 250px、スマホ: 200px × 200px）
    - 画面UI全体のサイズを拡大（max-width: 1600px）
  - スマホデザイン対応
    - レスポンシブデザインを追加（768px以下、480px以下）
    - プレビュー画面のスマホ対応
    - イベント編集画面のスマホ対応
    - 画像選択モーダルのスマホ対応
  - イベント再生画面の表示崩れ修正
    - DOM要素の読み込み完了を待つ処理を追加（同期的処理）
    - 画像の読み込み完了を待つ処理を追加
    - レイアウトの再計算を強制する処理を追加
    - 画面サイズの固定（拡大・縮小を防止）
  - その他の改善
    - 画面左上のタイトルをクリックでトップページに戻る機能を追加
    - イベント選択画面のローディング表示を追加
    - 新規追加したキャラクター画像を反映する機能（update_images_json.js）
    - フッターに素材提供元のクレジットを追加（KPB公式ウェブサイト）
    - フッターを画面下部に固定
